...
public class ExampleDao extends BaseDao {
	...
	public List<Example> getAllExamples(int type) {
		return getSession().createQuery("from Example where category_id=:type")
				.setParameter("type", type).list();
	}

	//获取部分案例信息
	public List<Example> getExampleList(int begin, int rows) {
		return getSession().createQuery("from Example order by id desc")
	...
	public Example getExampleById(int id) {
		return (Example) getSession().createQuery("from Example where id=:id")
