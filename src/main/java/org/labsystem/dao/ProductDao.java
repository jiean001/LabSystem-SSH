...
public class ProductDao extends BaseDao {
	...
	public List<Product> getAllProducts(int type) {
		return getSession().createQuery("from Product where category_id=:type").setParameter("type", type).list();
	}

	// 随机获取产品信息
	public List<Product> getProductList(int begin, int rows) {
		return getSession().createQuery("from Product order by id desc").setFirstResult(begin).setMaxResults(rows)
				.list();
	}

	// 随机获取部分产品信息
	public List<Product> getPartProductRandom(int begin, int rows) {
		return getSession().createQuery("from Product order by rand()").setFirstResult(begin).setMaxResults(rows)
				.list();
	}
	...
	public Product getProduct(int id) {
		return (Product) getSession().createQuery("from Product where id=:id").setParameter("id", id).uniqueResult();
	}
